 ...
 * Params for [[LSH]].
 */
private[ml] trait CustomizedLSHParams extends HasInputCol with HasOutputCol {
  /**
 ...
 */
private[ml] abstract class CustomizedLSHModel[T <: CustomizedLSHModel[T]]
 ...
 */
private[ml] abstract class CustomizedLSH[T <: CustomizedLSHModel[T]]
  extends Estimator[T] with CustomizedLSHParams with DefaultParamsWritable {
  self: Estimator[T] =>
  ...
  override def fit(dataset: Dataset[_]): T = {
    ...
    val model = createRawCustomizedLSHModel(inputDim).setParent(this)
    copyValues(model)
  }
}
